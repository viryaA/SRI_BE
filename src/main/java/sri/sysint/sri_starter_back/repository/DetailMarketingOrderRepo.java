...
public interface DetailMarketingOrderRepo extends JpaRepository<DetailMarketingOrder, BigDecimal>{
	...
	@Query(value = "SELECT \r\n"
			+ "    ITEM_CURING, \r\n"
			+ "    MAX(NUMBER_OF_MOULD) AS NUMBER_OF_MOULD, \r\n"
			+ "    SUM(CAV) AS TOTAL_CAV, \r\n"
			+ "    LISTAGG(RIM, ', ') WITHIN GROUP (ORDER BY NULL) AS LIST_RIM, \r\n"
			+ "    SUM(TOTAL_AR) AS TOTAL_TOTAL_AR \r\n"
			+ "FROM (\r\n"
			+ "    SELECT \r\n"
			+ "        P.ITEM_CURING, \r\n"
			+ "        IC.NUMBER_OF_MOULD, \r\n"
			...
			+ "        SRI_IMPP_M_PRODUCT P ON MO.PART_NUMBER = P.PART_NUMBER\r\n"
			+ "    LEFT JOIN \r\n"
			+ "        SRI_IMPP_M_ITEMCURING IC ON P.ITEM_CURING = IC.ITEM_CURING\r\n"
			+ "    LEFT JOIN \r\n"
			+ "        SRI_IMPP_M_PRODUCTTYPE PT ON P.PRODUCT_TYPE_ID = PT.PRODUCT_TYPE_ID\r\n"
			+ "    LEFT JOIN \r\n"
			+ "        ( \r\n"
			...
			+ "        COUNT_CT.COUNT_ITEM_CURING ASC, \r\n"
			+ "        CASE \r\n"
			+ "            WHEN PT.PRODUCT_TYPE = 'TL' THEN 1 \r\n"
			+ "            WHEN PT.PRODUCT_TYPE = 'TT' THEN 2 \r\n"
			+ "        ELSE 3 \r\n"
			+ "    END\r\n"
			+ ") GROUPED_DATA\r\n"
			+ "GROUP BY ITEM_CURING\r\n"
			+ "ORDER BY TOTAL_TOTAL_AR DESC", nativeQuery = true)	
